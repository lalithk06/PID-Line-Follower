...
// ============================================================
// activeSpeed passed in — pivot uses max(activeSpeed, 140)
// so adaptive speed applies but pivot never goes below 140
// ============================================================
bool handleJunction(int activeSpeed) {

  if(fabs(filteredError) > 0.7f) return false;
  ...
  int pivotSpeed = max(activeSpeed, 140);  // adaptive but never below 140
...
void pidControl(){
  ...
  // so all handlers benefit from adaptive speed
  ...
  // ── Special handlers — all receive activeSpeed ──
